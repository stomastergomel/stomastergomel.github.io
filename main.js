...
;// CONCATENATED MODULE: ./src/assets/js/anchor-animations.js
...
;// CONCATENATED MODULE: ./src/assets/js/application-form.js
...
const FORM_URL = "https://evgnpn.000webhostapp.com/mlsndr.php";
const TOKEN = "96efa474-587a-48c6-9ee3-2526082480d8";
const SEND_URL = FORM_URL + "?token=" + TOKEN;

const SUBJECT = "Заявка СТО МАСТЕР";
const TO_ADDRESS = "stomastergomel@yandex.by";
const TO_NAME = "СТО МАСТЕР";

const application_form_form = document.querySelector("#application-form");
const submitButton = application_form_form.querySelector("button[type=submit]");
...
submitButton.addEventListener("click", (ev) => {
  ...
  fetch(SEND_URL, {
    ...
    .then((resp) => {
      let modal;
      if (resp.ok) {
        modal = new Modal(document.getElementById("submitSuccessModal"));
      } else {
        modal = new Modal(document.getElementById("submitFailModal"));
      }
      modal.show();
      return resp.json();
    })
...
function validateForm() {
  ...
  isValid = validateNotEmpty(
    "input[name=fromName]",
    "#fromNameInvalidFeedback"
  );
  isValid = validateNotEmpty("input[name=fromTel]", "#fromTelInvalidFeedback");

  isValid = validateEmail('input[name=fromAddress]',"#fromAddressInvalidFeedback");

  return isValid;
}

function validateNotEmpty(controlSelector, invalidFeedbackSelector) {
  let isValid = true;
  const controlEl = application_form_form.querySelector(controlSelector);
  const invalidFeedbackEl = application_form_form.querySelector(invalidFeedbackSelector);
  if (!controlEl.value) {
    invalidFeedbackEl.classList.add("d-block");
    isValid = false;
  } else {
    invalidFeedbackEl.classList.remove("d-block");
  }
  return isValid;
}

function validateEmail(controlSelector, invalidFeedbackSelector) {

  const controlEl = application_form_form.querySelector(controlSelector);
  const invalidFeedbackEl = application_form_form.querySelector(invalidFeedbackSelector);

  if (!controlEl.value) return false;

  const isValid = (0,email_validator/* validate */.G)(controlEl.value);
...
function getFromName() {
  return application_form_form.querySelector("input[name=fromName]").value ?? "";
}

function getFromAddress() {
  return application_form_form.querySelector("input[name=fromAddress]").value ?? "";
}

function getFromTel() {
  return application_form_form.querySelector("input[name=fromTel]").value ?? "";
}

function getMessage() {
  return application_form_form.querySelector("textarea[name=message]").value ?? "";
}
...
function Email(subject, body, toAddress, toName, fromAddress, fromName) {
